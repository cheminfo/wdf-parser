...
test('getBlockTypes return value at different inputs', (): void => {
  ...
  expect(getBlockTypes(0x4e47524f)).toBe('WDF_BLOCKID_ORIGIN');
  expect(getBlockTypes(0x54584554)).toBe('WDF_BLOCKID_COMMENT');
  expect(getBlockTypes(0x41445857)).toBe('WDF_BLOCKID_WIREDATA');
  expect(getBlockTypes(0x42445857)).toBe('WDF_BLOCKID_DATASETDATA');
  expect(getBlockTypes(0x4d445857)).toBe('WDF_BLOCKID_MEASUREMENT');
  expect(getBlockTypes(0x53435857)).toBe('WDF_BLOCKID_CALIBRATION');
  expect(getBlockTypes(0x53495857)).toBe('WDF_BLOCKID_INSTRUMENT');
  expect(getBlockTypes(0x50414d57)).toBe('WDF_BLOCKID_MAPAREA');
  expect(getBlockTypes(0x4c544857)).toBe('WDF_BLOCKID_WHITELIGHT');
  expect(getBlockTypes(0x4c49414e)).toBe('WDF_BLOCKID_THUMBNAIL');
  expect(getBlockTypes(0x2050414d)).toBe('WDF_BLOCKID_MAP');
  expect(getBlockTypes(0x52414643)).toBe('WDF_BLOCKID_CURVEFIT');
  expect(getBlockTypes(0x534c4344)).toBe('WDF_BLOCKID_COMPONENT');
  expect(getBlockTypes(0x52414350)).toBe('WDF_BLOCKID_PCA');
  expect(getBlockTypes(0x4552434d)).toBe('WDF_BLOCKID_EM');
  expect(getBlockTypes(0x43444c5a)).toBe('WDF_BLOCKID_ZELDAC');
  expect(getBlockTypes(0x4c414352)).toBe('WDF_BLOCKID_RESPONSECAL');
  expect(getBlockTypes(0x20504143)).toBe('WDF_BLOCKID_CAP');
  expect(getBlockTypes(0x50524157)).toBe('WDF_BLOCKID_PROCESSING');
  expect(getBlockTypes(0x41524157)).toBe('WDF_BLOCKID_ANALYSIS');
  expect(getBlockTypes(0x4c424c57)).toBe('WDF_BLOCKID_SPECTRUMLABELS');
  expect(getBlockTypes(0x4b484357)).toBe('WDF_BLOCKID_CHECKSUM');
  expect(getBlockTypes(0x44435852)).toBe('WDF_BLOCKID_RXCALDATA');
  expect(getBlockTypes(0x46435852)).toBe('WDF_BLOCKID_RXCALFIT');
  expect(getBlockTypes(0x4c414358)).toBe('WDF_BLOCKID_XCAL');
  expect(getBlockTypes(0x48435253)).toBe('WDF_BLOCKID_SPECSEARCH');
  expect(getBlockTypes(0x504d4554)).toBe('WDF_BLOCKID_TEMPPROFILE');
  expect(getBlockTypes(0x56434e55)).toBe('WDF_BLOCKID_UNITCONVERT');
  expect(getBlockTypes(0x52505241)).toBe('WDF_BLOCKID_ARPLATE');
  expect(getBlockTypes(0x43454c45)).toBe('WDF_BLOCKID_ELECSIGN');
  expect(getBlockTypes(0x4c584b42)).toBe('WDF_BLOCKID_BKXLIST');
  expect(getBlockTypes(0x20585541)).toBe('WDF_BLOCKID_AUXILARYDATA');
  expect(getBlockTypes(0x474c4843)).toBe('WDF_BLOCKID_CHANGELOG');
  expect(getBlockTypes(0x46525553)).toBe('WDF_BLOCKID_SURFACE');
  expect(getBlockTypes(0xffffffff)).toBe('WDF_BLOCKID_ANY');
  expect(getBlockTypes(0x54455350)).toBe('WDF_STREAM_IS_PSET');
  expect(getBlockTypes(0x4d545343)).toBe('WDF_STREAM_IS_CSTM');
  expect(() => getBlockTypes(0x25)).toThrow('blockId 37 is not defined');
});
...
test("Experiment's Measurement Units", (): void => {
  ...
  expect(() => getMeasurementUnits(25)).toThrow(
    'Unexpected value for unit: 25',
  );
});
...
test('Word describing the type of spectra in the file', (): void => {
  ...
  expect(() => getOverallSpectraDescription(25)).toThrow(
    'Expected: 0,1,2 or 3. Got 25',
  );
});
