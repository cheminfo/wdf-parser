...
export function analyzeFileHeader(header: IOBuffer): object {
   ...
   */
  const signature = header.readUint32(); //typeId for block header
  const version = header.readUint32(); //version if it's file header, uid otherwise
  const size = Number(header.readBigUint64()); //512bytes for file header.
